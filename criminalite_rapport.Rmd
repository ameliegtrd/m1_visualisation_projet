...
load_lib <- c("tidyverse","readxl","lmtest","systemfit","ggcorrplot", "ggpmisc","cowplot","sandwich", "kableExtra", "knitr", "rAmCharts")
...
# data delits_fr_2016
readLines("delits_fr_2016.csv",2)
# on regarde l'encodage
guess_encoding("delits_fr_2016.csv")
# on importe
delits_fr_2016  <- read.csv("delits_fr_2016.csv", encoding = "utf-8", fileEncoding = "utf-8", sep=";",dec=".", header = T)
...
delits_fr_2016_2  <- delits_fr_2016  %>% select(-1) %>% t %>% as.data.frame() %>% mutate("Departement" = departement) %>% relocate("Departement", .before = 1)
# summary(delits_fr_2016_2)

## Donnees population
population  <- read_excel("donnees.xlsx", sheet ="Population", skip=7, col_names = T)
population  <- population %>% rename("Departement" = "Code département")
summary(population)

## Donnees pauvrete
pauvrete  <- read_excel("donnees.xlsx", sheet ="Pauvrete", skip=4, col_names = T)
pauvrete <- pauvrete %>% select(-2) %>% rename("Departement" = "Code géographique")
summary(pauvrete)

## Donnees diplome
diplome_inital <- read_excel("donnees.xlsx", col_names = T,sheet ="Diplome", skip=3)
diplome_inital <- diplome_inital  %>% rename("Departement" = "Département\r\nen géographie courante") %>% select(c(-1,-3:-6))
...
diplome <- diplome %>% group_by(Departement) %>% summarise(Aucun_diplome = sum(Aucun_diplome, na.rm=T),Niveau_CAP_BEP = sum(Niveau_CAP_BEP, na.rm=T),Niveau_BAC =sum(Niveau_BAC, na.rm = T) ,Etudes_sup = sum(Etudes_sup, na.rm = T)) 
...
revenu_median  <- read_excel("donnees.xlsx", sheet ="Revenu_median_Gini", skip=4, col_names = T)
revenu_median <- revenu_median %>% rename("Departement" = "Code géographique") %>% select(-2)

## Donnees taux de chomage
taux_chomage  <- read_excel("donnees.xlsx", sheet ="Taux_chomage", col_names = T)
...
delits_fr_2016_2 %>% summarise_all(~ sum(is.na(.))) %>% View() # pour chaque colonne
...
delits_fr_2016_final <- delits_fr_2016_2 %>% left_join(y = population,by="Departement") %>% left_join(y=pauvrete, by="Departement") %>% left_join(y=diplome, by="Departement") %>% left_join(y=revenu_median, by="Departement") %>% left_join(y=taux_chomage, by="Departement")
...
- **Statistiques descriptives** : Cette partie comporte deux onglets. 
  - **Premier onglet** : Il permet de choisir le type de criminalité, et d'afficher par département le nombre total de crime. Il fait de même pour 100 000 habitants. Il présente aussi des boxplots et des tableaux contenant des statistiques telles que la moyenne, la médiane, etc. 
  - **Second onglet** : Il permet de choisir un département et d'afficher un "pie graph" avec les pourcentages de chaque type de crime. L'utilisateur aura aussi la possibilité de comparer deux départements. 
...
criminalite %>% slice_max(Nb_delits_100000hab,n=10) %>% select("Departement","Nb_delits_100000hab") %>% View()
```
...
Département  | Nb_delits_100000hab | 
------------- | ------------- | 
75 |  12 403 | 
75 |  12 403 | 
75 |  12 403 | 
...
my.formula <- y ~ x
htp <- ggplot(criminalite, aes(x = Nb_delits_100000hab, y = Tx_pauvrete_seuil60, label=Tx_pauvrete_seuil60)) + geom_smooth(method=lm, se=F, formula = my.formula) + stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), parse = TRUE) + geom_point() 

## nuage de points entre criminalite et part de non diplomes
hpnd <- ggplot(criminalite, aes(x = Nb_delits_100000hab, y = Part_non_diplome, label = Part_non_diplome)) + geom_smooth(method=lm, se=F, formula = my.formula) +  stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), parse = T) + geom_point() 

## nuage de points entre criminalite et indice de Gini
hig <- ggplot(criminalite, aes(x = Nb_delits_100000hab, y = Indice_gini, label=Indice_gini))  +  geom_smooth(method=lm, se=F, formula = my.formula) + stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), parse = T) + geom_point()

## nuage de points entre criminalite et taux chomage moyen
htcm <- ggplot(criminalite, aes(x = Nb_delits_100000hab, y = Taux_chomage_moyen, label=Indice_gini))  +  geom_smooth(method=lm, se=F, formula = my.formula) + stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), parse = T) + geom_point()

# les 4 graphes sur la meme fenetre
...
```
**Graphique :** A priori, il y a corrélation positive entre l'indice de Gini et le nombre de délits pour 100 000 habitants car la pente de la droite de régression est clairement positive.
**Corrplot :** Il existe une relation fortement positive entre le nombre délits pour 100 000 habitants et l'indice Gini (0.78). Il y a aussi une relation positive élevée entre le taux de pauvreté (au seuil de 60%) et le taux de chômage moyen (0.76) 


ainsi qu'entre le taux de pauvreté et la part de non diplômés (0.62). A l'inverse, relation négative entre le salaire médian et la part de non diplômés (-0.78). Attention, le graphe de corrélation ne permet pas d'affirmer un lien de causalité entre les variables. Les indicateurs de corrélations aident à formaliser le modèle mais on ne peut pas parler de lien de causalité. 
